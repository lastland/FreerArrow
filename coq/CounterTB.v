...
Section CounterTLaws.
    ...
    Hint Unfold left ChoiceProfunctor__Fun ChoiceProfunctor__CounterT : counterT.
    ...
    Proof.
      ...
      constructor; autounfold with counterT in *.
      - (* left_arr *)
        intros. rewrite !dimap_arr_comp, !left_arr, <- !arr_distr.
        apply Harr. extensionality a. fcrush.
      - (* left_distr *)
        intros. rewrite !dimap_arr_comp, !left_distr, !comp_assoc.
        rewrite <- (comp_assoc (B * nat + D * nat) ((B + D) * nat) _ ((C + D) * nat)), <- arr_distr.
        repeat (apply Hcomp; [reflexivity |]).
        remember (fun a : B * nat + D * nat => _) as func1.
        assert (func1 = (fun x => x)).
        { extensionality a. fcrush. }
        rewrite H0, arr_id, left_id. reflexivity.
      - (* left_inl *)
